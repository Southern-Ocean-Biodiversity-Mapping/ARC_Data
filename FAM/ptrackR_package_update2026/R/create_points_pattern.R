...
create_points_pattern <- function(raster, multi = 1, mld = 0){
  ...
  require(spatstat)
  require(maptools)
  chl_image <- maptools::as.im.RasterLayer(raster*multi)  #converts raster into an image with a multiplicator
  ptspattern <- spatstat::rpoispp(chl_image)      #random poisson point process transform values into probability-density of points
