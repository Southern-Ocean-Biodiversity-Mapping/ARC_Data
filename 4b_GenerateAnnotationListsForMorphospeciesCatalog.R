...
cover_list2<-read_xlsx(path=paste0(ARC_Data.dir, "Annotation/Species_list_vs_CATAMI_2023_01.xlsx"),sheet=1)
count_list2<-read_xlsx(path=paste0(ARC_Data.dir, "Annotation/Species_list_vs_CATAMI_2023_01.xlsx"),sheet=2)
...
ann.cover.dat.temp <- ann.cover.dat2 %>%
  left_join(cover_list2[,c("Label", "Name_to_publish", "CATAMI_broad", "CATAMI", "CAAB")], by=c("Label_after_review_clean"="Label"))
...
write.csv(ann.cover.dat3,file=paste0(cover.path,"Circumpolar_DownwardImages_PointScore_Annotations_202301_rawforpublication.csv"))
...
factor.cols <- c("Annotator","Label","Label_after_review","Label_after_review_clean","Name_to_publish","CATAMI","CAAB","SurveyID")
ann.cover.dat4[,factor.cols] <- lapply(ann.cover.dat4[,factor.cols], factor)
ann.cover.dat <- ann.cover.dat4

save(ann.cover.dat, file=paste0(cover.path,"Circumpolar_DownwardImages_PointScore_Annotations_202301.Rdata"))
...
ann.count.dat.temp <- ann.count.dat2 %>%
  left_join(count_list2[,c("Label", "Name_to_publish", "CATAMI_broad", "CATAMI", "CAAB")], by=c("Label_clean"="Label"))
