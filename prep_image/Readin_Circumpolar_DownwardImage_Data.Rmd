...
for(i in 1:length(dat.subset.list)){
  ...
  if (sum(is.na(dat.subset.list[[i]]$area))) {
    ...
    if (sum(is.na(dat.subset.list[[i]]$area)) == nrow(dat.subset.list[[i]])) {
      # If no area value available at all, estimated_area filled with NA
      dat.subset.list[[i]]$estimated_area <- NA
    }
    else {
      # Get indexes
      idx_NA_values <- which(is.na(dat.subset.list[[i]]$area))
      ...
      avg_value <- mean(dat.subset.list[[i]]$area[idx_nonNA_values])
      # Assign this value to images with NA area
      dat.subset.list[[i]]$estimated_area[idx_NA_values] <- avg_value
      
      # Assign true area for images with non NA area
      dat.subset.list[[i]]$estimated_area[idx_nonNA_values] <- dat.subset.list[[i]]$area[idx_nonNA_values]
    }
