  ...
  if (sum(is.na(dat.subset.list[[i]]$area))) {
    ...
    for (transect_id in transect_ids) {
      ...
      if (sum(is.na(dat_transect$area)) == nrow(dat_transect)) {
        ...
        dat.subset.list[[i]][idxes_transet, ]$estimated_area <- NA
        print(paste("survey: ", names(dat.subset.list)[[i]] , " / Transect: ", as.vector(unique(dat_transect$transectID))))
      }
      else {
        # Get indexes
        ...
        dat.subset.list[[i]][idxes_transet, ]$estimated_area[idx_NA_values] <- avg_value
    
        # Assign true area for images with non NA area
        dat.subset.list[[i]][idxes_transet, ]$estimated_area[idx_nonNA_values] <- dat.subset.list[[i]][idxes_transet, ]$area[idx_nonNA_values]
      }
    }
    }
  }
